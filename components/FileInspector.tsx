    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 p-4">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-4xl max-h-[90vh] flex flex-col">
        ...
        <div className="px-6 py-4 border-b border-gray-200 flex justify-between items-center bg-gray-50 rounded-t-lg">
          <div>
             <h3 className="text-xl font-bold text-gray-900">
                 {data.type === 'Sales' ? `Ticket Inspection: ${data.ticketNum}` : `Summary Inspection: ${data.ticketNum}`}
             </h3>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-8">
            ...
            <div>
                <h4 className="text-lg font-medium text-gray-800 mb-4 pb-1 border-b">
                    {data.type === 'Sales' ? 'Internal Consistency (Header vs Lines)' : 'Internal Consistency (Header vs Subfamilies)'}
                </h4>
                        ...
                        <div key={idx} className={`p-4 rounded-lg border ${check.isOk && !check.isWarning ? 'border-green-200 bg-green-50' : check.isWarning ? 'border-yellow-200 bg-yellow-50' : 'border-red-200 bg-red-50'}`}>
                            ...
                            <div className="text-xs space-y-1">
                                ...
                                <div className="flex justify-between border-b border-gray-300 pb-1">
                                    <span className="text-gray-500">{data.type === 'Sales' ? 'Sum Lines:' : 'Sum Body:'}</span>
            ...
            {/* 2. Line Items Table (Sales) */}
            {data.type === 'Sales' && data.lines && (
                <div>
            ...
            {/* 2. Subfamily Aggregations Table (Summary) */}
            {data.type === 'Summary' && data.aggregations && (
                <div>
                    <h4 className="text-lg font-medium text-gray-800 mb-4 pb-1 border-b">Subfamily Aggregations (Body)</h4>
                    <div className="overflow-auto border rounded-lg max-h-96 relative shadow-inner">
                        <table className="min-w-full text-xs divide-y divide-gray-200">
                            <thead className="bg-gray-100 sticky top-0 z-10 shadow-sm">
                                <tr>
                                    <th rowSpan={2} className="px-3 py-2 text-left font-medium text-gray-500 bg-gray-100 border-r">SubFam</th>
                                    <th colSpan={3} className="px-3 py-1 text-center font-medium text-gray-500 border-b bg-blue-50">Sales (V)</th>
                                    <th colSpan={3} className="px-3 py-1 text-center font-medium text-gray-500 border-b bg-amber-50">Returns (D)</th>
                                </tr>
                                <tr>
                                    <th className="px-2 py-1 text-right font-medium text-gray-500 bg-blue-50">Qty</th>
                                    <th className="px-2 py-1 text-right font-medium text-gray-500 bg-blue-50">Gross</th>
                                    <th className="px-2 py-1 text-right font-medium text-gray-500 bg-blue-50 border-r">Disc</th>
                                    <th className="px-2 py-1 text-right font-medium text-gray-500 bg-amber-50">Qty</th>
                                    <th className="px-2 py-1 text-right font-medium text-gray-500 bg-amber-50">Gross</th>
                                    <th className="px-2 py-1 text-right font-medium text-gray-500 bg-amber-50">Disc</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-gray-200 bg-white">
                                {data.aggregations.map((agg, idx) => (
                                    <tr key={idx} className="hover:bg-gray-50">
                                        <td className="px-3 py-2 text-gray-900 font-bold border-r">{agg.TIPO_SUBFAMILIA}</td>
                                        <td className="px-2 py-2 text-right text-gray-600 bg-blue-50/30">{agg.ARTICULOS_V}</td>
                                        <td className="px-2 py-2 text-right font-mono text-blue-700 bg-blue-50/30">{fmtMoney(agg.IMPBRUTO_VSFZ)}</td>
                                        <td className="px-2 py-2 text-right font-mono text-blue-900 bg-blue-50/30 border-r">{fmtMoney(agg.IMPDESCUENTO_VSFZ)}</td>
                                        <td className="px-2 py-2 text-right text-gray-600 bg-amber-50/30">{agg.ARTICULOS_D}</td>
                                        <td className="px-2 py-2 text-right font-mono text-amber-700 bg-amber-50/30">{fmtMoney(agg.IMPBRUTO_DSFZ)}</td>
                                        <td className="px-2 py-2 text-right font-mono text-amber-900 bg-amber-50/30">{fmtMoney(agg.IMPDESCUENTO_DSFZ)}</td>
                                    </tr>
                                ))}
                            </tbody>
                            <tfoot className="bg-gray-50 font-bold sticky bottom-0 z-10 border-t shadow-[0_-1px_2px_rgba(0,0,0,0.05)]">
                                <tr>
                                    <td className="px-3 py-2 text-right text-gray-700 border-r">TOTALS:</td>
                                    <td className="px-2 py-2 text-right text-gray-900 bg-blue-50/30">{data.aggregations.reduce((a, b) => a + b.ARTICULOS_V, 0)}</td>
                                    <td className="px-2 py-2 text-right text-gray-900 bg-blue-50/30">{fmtMoney(data.aggregations.reduce((a, b) => a + b.IMPBRUTO_VSFZ, 0))}</td>
                                    <td className="px-2 py-2 text-right text-gray-900 bg-blue-50/30 border-r">{fmtMoney(data.aggregations.reduce((a, b) => a + b.IMPDESCUENTO_VSFZ, 0))}</td>
                                    <td className="px-2 py-2 text-right text-gray-900 bg-amber-50/30">{data.aggregations.reduce((a, b) => a + b.ARTICULOS_D, 0)}</td>
                                    <td className="px-2 py-2 text-right text-gray-900 bg-amber-50/30">{fmtMoney(data.aggregations.reduce((a, b) => a + b.IMPBRUTO_DSFZ, 0))}</td>
                                    <td className="px-2 py-2 text-right text-gray-900 bg-amber-50/30">{fmtMoney(data.aggregations.reduce((a, b) => a + b.IMPDESCUENTO_DSFZ, 0))}</td>
                                </tr>
                            </tfoot>
                        </table>
                    </div>
            ...
            {/* 3. Tax Breakdown (6xx) - Sales Only */}
            {data.type === 'Sales' && data.taxes && (
                <div>
            ...
            {/* 4. Payment Methods (7xx) - Sales Only */}
            {data.type === 'Sales' && data.payments && (
                <div>
